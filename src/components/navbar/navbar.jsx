...
import { BiExit, BiHome, BiSelection, BiSolidHome } from "react-icons/bi";
...
function NavBar({ setMostrar, setIsVisible }) {
  ...
  useEffect(() => {
    ...
    socket.emit("sendNotify", sessionStorage.getItem("id"));
  }, []);
        ...
        <li className="home focus" onLoad={(e) => toggle(e)}>
          <div className="nos">
            <Link
              ...
              <BiSolidHome size={"20px"} className="link-nav" color="fff" />
            </Link>
          </div>
        </li>
